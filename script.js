...
const sliderLineRight = document.querySelector(".slider-line-right");
...
function doScroll() {
  ...
  for (let dotsItem of dots) {
    dotsItem.classList.remove("active");
  }
  dots[count].classList.add("active");
}
function doScrollRight() {
  sliderLineRight.style.transform = "translateY(-" + count * 100 + "vh)";
}

function rollSlideDown() {
  count++;
  if (count >= block.length) {
    count = 0;
  }
  doScroll();
  setTimeout(doScrollRight, 200);
}

function rollSlideUp() {
  count--;
  if (count < 0) {
...
buttonSliderNext.addEventListener("click", rollSlideDown);
...
(function (window) {
  let canvas,
    ...
    FPS = 60,
    words = ["Hi my name is Aleks", "ovals", "drop", "ribbon"],
    colors = {
      circle: ["#9932CC", "#FF8C00", "#006400", "#FFD700"],
      ovals: ["#c0392b", "#ff7e15"],
      drop: ["#1d75cf", "#3a5945"],
  ...
  function init() {
    let slideshowContainer = document.querySelector(".ip-slideshow");
  ...
  function updateTransition() {
    ...
    if (nextText.length < text.length) {
      ...
      for (var index = 0; index < extra.length; index++) text.splice(index, 1);
    }
  }
  ...
  function update() {
    ...
    [].forEach.call(text, function (particle, index) {
      ...
      if (particle.hasBorn) {
        ...
        if (Math.round(particle.radius) === 0) particle.hasBorn = false;
      }
